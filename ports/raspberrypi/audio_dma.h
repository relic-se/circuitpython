...
typedef struct {
    mp_obj_t sample;
    uint8_t *output_buffer[2]; // Allocated through port_malloc on RP2350 so they are dma-able
    size_t output_buffer_length[2];
    uint8_t *input_buffer[2]; // Allocated through port_malloc on RP2350 so they are dma-able
